...
T[43]={name:'a tree stands in blocks: the axe bites the bole and it gives timber',
  ...
  run:async page=>page.evaluate(async()=>{
    ...
    const b=D.blockOf(n);
    /* ---- TIMBER IS WHAT DROPS TIMBER, not one block id ----
       Written `b.id==='log'` when Timber was one block; Round 86 gave every
       bark its own Timber block so a felled birch keeps its birch — and all
       six DROP plain 'log', which is the thing this question was ever really
       asking: does the bole break into timber, or into rubble. The stamp
       still answers `stone` for a material no block claims, and stone drops
       stone, so the rubble fault this guards is caught exactly as before. */
    const isTimber=!!(b&&b.drops==='log');
    ...
    if(six.chunks!==one.chunks)
      ...
      if(Math.abs(six.tris-one.tris)>triBand)
        faults.push('the geometry moved by '+(six.tris-one.tris)+
          ' triangles, past the '+Math.round(triBand)+' the trunk-boundary faces are priced at');
    }
    return {ok:!faults.length,
...
T[58]={name:'a hand sows: the hoe tills, the seed goes in on tilled ground only, and the corn stands',
  /* §17.4, THE DEBT IN ITS OWN WORDS: "a seed in the satchel, ground that
     will take it, and a crop that comes on where the traveller put it rather
     than where a village put it." The clock and the growth stood waiting
     since Round 68 — the whole agricultural year is a vertex-shader function
     of latitude and one uniform, and any cross drawn in the crop material
     gets it free — so this round wired the three verbs and nothing else:
     the hoe tills (a ground block's own `hoed` names what it becomes), the
     seed sows (its `sows` names the crop kind; the sown block is the ground
     id + the kind, which is how blocks/soil-wheat.js names itself), and the
     mesher draws ONE cross of that kind over every sown cell with the
     village fields' own material.

     THE SEASON RULE IS THE SHADER, so it is not re-proved here: sown any
     day, the crop stands sunk until the latitude's own seedtime — test 48
     already holds the GLSL to the JS twin at 120 points and holds the year
     to zero rebuilds; a hand-sown cell rides the same two laws. The drop's
     taking-up is test 15's; the reload of an EDITS deed is test 46's; both
     are said here rather than re-walked.

     THE GROUND IS A NAMED SPOT, as test 43's tamarisk is: the world is one
     seed, so the sward at this spot is the same sward every run, and a hunt
     from wherever the last test left the traveller found sand as often as
     grass. */
  run:async page=>page.evaluate(async()=>{
    const D=window.__VDBG, B=D.B;
    if(!D.placeFrom||!D.holdId||!D.workMake) return {pending:'no place/hold/work probes'};
    if(!D.blockId('seed-wheat')||!D.blockId('soil-wheat'))
      return {pending:'no seed in blocks/ (§17.4)'};
    /* known grassland, and the ground laid before anything is asked of it */
    const wx=12121.5*B, wz=2040.5*B;
    D.setMode('walk'); D.state.walk.x=wx; D.state.walk.z=wz; D.state.walk.feetY=undefined;
    for(let f=0;f<90;f++){ D.updateChunks(wx,wz,900);
      await new Promise(r=>requestAnimationFrame(r)); }
    const p=D.playerXZ();
    let g=null;
    for(let r=1;r<40&&!g;r++) for(let a=-r;a<=r&&!g;a++) for(let b2=-r;b2<=r;b2++){
      if(Math.max(Math.abs(a),Math.abs(b2))!==r) continue;
      const qx=Math.floor(p.x/B)+a, qz=Math.floor(p.z/B)+b2;
      const cc=D.landAtWorld((qx+0.5)*B,(qz+0.5)*B); if(!cc) continue;
      const n=D.blockAt(qx,cc.h-1,qz), bl=n?D.blockOf(n):null;
      if(bl&&bl.hoed&&!D.blockSolidAt(qx,cc.h,qz)){ g={qx,qz,y:cc.h-1,ground:bl,n}; break; }
    }
    if(!g) return {ok:false,got:'no ground that tills within forty blocks of the sward'};
    const aim=()=>({ix:g.qx,iy:g.y,iz:g.qz,nx:0,ny:1,nz:0,n:D.blockAt(g.qx,g.y,g.qz),dist:2});
    const faults=[];

    /* ---- 1 · THE SEED, OUT OF THE THRESHING — the economy, not a gift ---- */
    D.satchelAdd('hay',1);
    const th=D.workMake('thresh-wheat');
    const seeds0=D.satchelCount('seed-wheat');
    if(!th||!th.ok) faults.push('the threshing would not give: '+((th&&th.why)||'?'));
    else if(seeds0<2) faults.push('one sheaf threshed to '+seeds0+' of seed, wanted 2');

    /* ---- 2 · UNTILLED GROUND REFUSES, and keeps the seed ---- */
    D.holdId('seed-wheat');
    const refusal=D.placeFrom(aim());
    if(!refusal||!refusal.no) faults.push('untilled ground took the seed: '+JSON.stringify(refusal));
    if(D.satchelCount('seed-wheat')!==seeds0)
      faults.push('the refusal cost him seed');

    /* ---- 3 · THE HOE TILLS — and refuses the rock (the injection) ---- */
    D.satchelAdd('flint-hoe',1); D.holdId('flint-hoe');
    const rock=D.placeFrom({ix:g.qx,iy:0,iz:g.qz,nx:0,ny:1,nz:0,n:D.blockAt(g.qx,0,g.qz),dist:2});
    const rockHeld=!!(rock&&rock.no);
    const tilled=D.placeFrom(aim());
    const soilStands=(D.blockOf(D.blockAt(g.qx,g.y,g.qz))||{}).id==='soil';
    if(!tilled||!tilled.tilled) faults.push('the hoe would not till '+g.ground.id+': '+JSON.stringify(tilled));
    if(!soilStands) faults.push('the tilled cell does not answer soil');
    if(!rockHeld) faults.push('the hoe tilled the DEEP ROCK — hoed is not being asked');

    /* ---- 4 · THE SEED GOES IN, at a cost, and the deed is recorded ---- */
    D.holdId('seed-wheat');
    const sown=D.placeFrom(aim());
    const sownBlock=(D.blockOf(D.blockAt(g.qx,g.y,g.qz))||{}).id;
    if(!sown||!sown.sown) faults.push('tilled ground refused the seed: '+JSON.stringify(sown));
    if(sownBlock!=='soil-wheat') faults.push('the sown cell answers '+sownBlock+', wanted soil-wheat');
    if(D.satchelCount('seed-wheat')!==seeds0-1)
      faults.push('the sowing cost '+(seeds0-D.satchelCount('seed-wheat'))+' seed, wanted 1 (a voyage pays)');
    if(!(D.recordedAt&&D.recordedAt(g.qx,g.y,g.qz)))
      faults.push('the sown ground is not in the record — the deed would not survive a reload');

    /* ---- 5 · THE CORN STANDS: the chunk's mesh gains the crop material ---- */
    const cropMeshes=()=>D.viewStats().byMat.filter(m=>/^crop/i.test(m[0]))
      .reduce((t,m)=>t+m[1],0);
    D.flushNow(); await new Promise(r=>requestAnimationFrame(r));
    const stood=cropMeshes();
    if(!(stood>0)) faults.push('no crop material stands anywhere in view — the corn is not drawn');
    /* and the year that rules it, read off the same twin test 48 proves */
    const latN=1-(Math.hypot((g.qx+0.5)*B,(g.qz+0.5)*B)/D.R_WORLD)*2;
    const Y=window.CROP&&CROP.yearAt?CROP.yearAt(0.30,latN):null;

    /* ---- 6 · BROKEN, THE GROUND GIVES THE SEED BACK ---- */
    const drops0=(D.drops()||[]).length;
    D.mineDrive(true); D.mineAt(g.qx,g.y,g.qz,0,1,0); D.mineHold(true);
    let broke=false;
    for(let k=0;k<200;k++){ D.mineStep(1/60);
      if(!D.blockSolidAt(g.qx,g.y,g.qz)){ broke=true; break; } }
    D.mineHold(false); D.mineAt(null); D.mineDrive(false);
    const dr=(D.drops()||[]).slice(drops0);
    const gaveSeed=dr.some(d=>d.id==='seed-wheat');
    if(!broke) faults.push('the sown ground would not break to the hand');
    if(broke&&!gaveSeed) faults.push('the broken ground gave no seed back ('+
      dr.map(d=>d.id).join(',')+')');

    /* the world is left as it was found: the sward back, which the record
       drops of itself (an edit that restores what would stand is not kept) */
    D.setBlock((g.qx+0.5)*B,(g.y+0.5)*B,(g.qz+0.5)*B,g.n);
    await D.settle(1);
    return {ok:!faults.length,
      got:'threshed 1 sheaf into '+seeds0+' of seed · untilled ground refused and kept the seed · '+
        g.ground.id+' tilled to soil · the deep rock refused the hoe · sown: '+sownBlock+
        ' at one seed of cost, in the record · '+stood+' crop mesh(es) stand in view · '+
        (Y?('the year at this latitude: sow '+Y.sow.toFixed(2)+' ripe '+Y.ripe.toFixed(2)+' (test 48 holds the shader to this)'):'no year twin')+
        ' · broken, it gave '+(gaveSeed?'the seed back':'nothing')+
        (faults.length?' · FAULTS: '+faults.join(' · '):'')};
  })};
