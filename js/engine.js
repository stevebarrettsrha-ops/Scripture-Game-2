...
function updateSeaFloor(px,pz,force){
  ...
  while(J.pass===0&&J.j<SB_N){
    ...
    for(let i=0;i<SB_N;i++){
      ...
      const dm=(SEA_SURF-J.h[c])/U_PER_M;          /* the depth of this block, in metres */
      const n1=fbm(wx*0.03+3,wz*0.03+7), n2=fbm(wx*0.011-5,wz*0.011+2);
      let kind=0;
      if(dm>1000) kind=3;                          /* below the last of the light — dark rock and ooze */
      else if(n2>0.60) kind=2;
      else if(n2<0.38) kind=1;
      if(n1>0.74&&dm<60) kind=4;                   /* the reef floor of the clear shallows */
      J.k[c]=kind;
    }
  ...
  while(J.j<SB_N){
    ...
    for(let i=0;i<SB_N;i++){
      ...
      /* the light does not reach the trenches: it is all but gone by 200 m
         and wholly gone by 1,000, so the floor darkens on its true measure */
      const dm2=(SEA_SURF-y)/U_PER_M, lit=0.30+0.62*Math.max(0,1-dm2/900);
