...
Phase 7 was taken out of order because it was asked for. This was the earliest phase still
open, and **two of its twelve items were already done** — so before planning anything I
went and looked, rather than planning work that exists.

> ## ✅ **PHASE 6 IS CLOSED — Round 81.** Eleven of the twelve items are built and measured;
> item 5 is part-built and every part of it measured. **Nothing in it stands on an unmeasured
> ✅.** Three things are measured-and-NOT-built and are named rather than dropped: the matriarch
> LEADING (five attempts, five reverts, the fifth failing with the mechanism demonstrably
> firing), the lion's rest (re-opened at 18–26% still), and flock-mates flying in STEP (birds
> now flock; that they fly in step is not established). Cost, all measured: the herd pass
> 0.20–0.43 ms a frame, the lie-up and the flocking both inside the run-to-run frame drift.
> AUDIT §4ce.
...
| 5. Herd and flock structure | **partial, and one clause more of it now stands.** Flight distance and the watch ✅ *Round 54*. Herds exist ✅ *Round 71*. **The young held at the centre** ✅ *Round 77* — and it took building the diagnostic Round 72 asked for, which found that the wander-target picker, the only cohesion this world ever had and the lever two earlier rounds built on, fires **0.0000 times a beast-second**: it is unreachable code for a beast standing in grass. Hung instead at the end of a mouthful, the mothers' rank goes from +0.00 to **+0.12** over four lands, the reach from 1.72 herd-radii to 1.00, and the mothers-against-others gap from −0.03 to **+1.04**. The station ring is capped to fit inside the radius the world gathers a herd within — uncapped it pulled a buffalo herd a hundred units across against a HERD_R of eighty, and test 35's herd-samples fell from 597 to 423 before the cap put them back. HOW FAR back is not a thing that statistic can say — capped it reads 485, 578 and 610 on three runs with nothing changed between them, against 597 before — so what is claimed is only that the uncapped 423 falls below every capped reading and below the before-value. **And the herds TRAVEL now**, which was not aimed at: test 50's centroid figure, flat at 3 units for four rounds and called by its own comment "the decisive one", reads **8, 12 and 23 units** on three readings against 3 before, with Tanzania’s centroid moving 74 units in a spell. Nothing about how BIG a herd is has been shown to change — mean size read 4.39, 3.94 and 3.85 against 3.93, so that claim was struck when the second reading came in. **And test 50’s depth line reads a NULL** — mothers 2.12 of a herd-radius out against others 2.06 — where the A/B reads +1.04 the other way; test 50 disclaims that statistic in its own comment, and test 53 has now measured the station lever at ~0.05 a beast-second against test 50’s 70-frame settle, so its window is too short for the rule to act in. The ✅ rests on the A/B (control and treatment alternating in one boot, 900-frame settle) and the photograph; lengthening test 50’s settle past that measured interval is what would settle the disagreement, and is left for whoever takes the item up. It is still not *matriarch-led* — nothing leads and the spawn tether is untouched — but the flat travel figure had been read as evidence that a tethered world cannot move a herd at all, and it is not. **Still NOT built: the matriarch LEADING.** *(Round 80 measured the stated reason and it is the wrong one. "Every beast is tethered to its spawn point for life" is true in EFFECT — a beast sits 21–28u from its birth ground and only 4–7 of ~80 ever leave the 84u disc — but there is no leash: the only movement constraint from `a.hx,a.hz` is the wander picker Round 77 measured firing 0.0000 times a beast-second, which is dead code. **Nothing holds a herd in place; nothing MOVES it.** Beasts are stationary 77–85% of frames, so the herd's centre does a slow random walk and that is all. What is wanted is not a leash cut but a HEADING given, which is a smaller and more tractable thing than four rounds have written down. NOTE two readings of this were thrown away before the one above: the first said the opposite — mean drift 73–81u, outliers past 2,000u — and was the respawn artefact, LANDLIFE being a fixed pool so a reaped beast returns as the same object with a stale mark; the second was taken in a world frozen at the container's own 3.9 a.m., because a bare `setLocalHour` is read back over four times a second while the course of the day stands on 'live'. **A MATRIARCH WAS THEN BUILT AND REVERTED TWICE MORE — the fifth and SIXTH attempts, and the first two to fail with the mechanism demonstrably FIRING.** Rank by young-at-foot, the herd's station ring set about her instead of the mob's middle so the following came free off Round 77's lever, a held bearing turned a little each march. 10 and 12 marches in two lands, and: travel 1.54→**0.78**→1.08 and 0.90→**0.75**→0.66 herd-radii, reach 1.07→**1.23**→0.92 and 0.89→**1.17**→0.83. No land travels further; both lands' reach gets worse. And travel already reads 0.66–1.54 radii with the lead OFF where four rounds recorded "3 units, 0.13 radii" — Round 77 moved it and nobody noticed. **Round 82 then tried the opposite mechanism** — no target moved at all, the matriarch owning a BEARING and every member's STEP bent along it, a shared velocity rather than a chased mark. It fired 4,377 and 1,298 times and reach was NOT damaged this time, but travel did not move; and the deciding sweep says why. Asked not "what should ship" but "has this mechanism the AUTHORITY to move a herd at all", bend 0→0.30→0.60→0.90 gives travel **0.68→0.57→0.51→0.48 radii** — falling MONOTONICALLY as the rule's authority rises, with both off-arms bracketing every on-reading. A characterised negative, the first §2.3.5 has produced. **What is now known:** moving the assigned marks fails because the re-aim rate is 0.05 a beast-second; leaning every step fails because the lean costs more in correction than it gains in displacement. A seventh attempt should move neither the marks nor the steps but change what a beast WANTS — the grazing choice, the only lever ever measured firing at a useful rate. ~~Written down, not attempted.~~ **BUILT — and after the seventh attempt characterised the measurement it needed, THE EIGHTH SHIPPED IT (Round 92): at 180-second arms with the cohort-fixed metric, both ON arms beat the off arm on travel (1.84 and 1.16 herd-radii against 0.84, a fourth independent reading at 2.50 agreeing) with reach, the mothers' place, the feeding and the frame cost flat across arms — the bracketing standard every earlier attempt failed. One land measured, said plainly; tests 50 and 53 stand watch. The herds of this earth walk their grazing forward under a matriarch's bearing at last. The seventh attempt's record follows, kept as the history it is:** ATTEMPTED — Round 89 — and taken back out with the first characterisation that convicts the INSTRUMENT and not the idea. The mechanism was the prescription to the letter: a matriarch by rank owning a slowly-wheeling bearing, and at the meal-done decision a beast that would feed in place sometimes wanting its next mouthful a couple of body lengths her way — stations untouched, steps untouched. Hung after `toStation` it was dead (0.000–0.002 a beast-second, the fourth dead lever this item has produced); re-hung at the feedhead expiry itself it FIRED, 0.13–0.23 a second with the off-arm at exactly 0 — **and cost nothing**: reach 1.14/1.10/1.09 across ON/off/ON, mothers 0.85–0.86 inside, feed flat, frame cost flat. But travel read 12.9/17.6/57.7 units with the OFF arm BETWEEN the ON arms, and the arithmetic says the arm cannot see the effect it asks about: ~7 marches of ~9 units over 35 beasts is ~2 units of expected drift under 13–18 of arrival noise. A 30-second arm is an order of magnitude too short — test 50's own complaint about itself, met again from the other side. **What the eighth attempt inherits:** the right hang point (the feedhead expiry, before the act and the stroll), a lever that demonstrably fires there for free, a cohort-fixed travel metric (the first one read 125 units of membership churn in an arm where the lever never fired once), and the requirement of MINUTES-long arms. Nothing disproved, nothing proved, nothing shipped. AUDIT §4cn.)* — ~~**and the birds' flocking**~~ — **BUILT in Round 81.** It had failed for two reasons at once and both were already-known shapes: there was **nothing to flock with** (24 birds for the whole earth over a 1,150u ring, each set down at an independent random point — 75% and 96% of flocking-bird-frames had no mate of their kind within 120u, which is Round 71's beast finding word for word, mended Round 71's way), and **the rule was nearly unreachable** (it lived in `rest` alone, 0.6–1.4% of frames — Round 77's dead-lever shape — and is asked in the HUNT now, by passing the flock's middle to `forageSpot` as its search centre so every ground check is the one that function always made). A/B/A in one boot: a bird with one of its own by it 13→**80**→36% and 0→**54**→13%; **biggest company 3→12→3 and 0→3→1**; frame cost inside the drift. **NOT claimed: that flock-mates fly in STEP** — the heading read 100→45→42° and —→29→39°, the off-again arm as tight as the on arm, so the reading is reported and is evidence for nothing |
| 6. The daily round | ✅ *Round 69, and MEASURED at last in Round 78*. Round 69 mended the one clause with nothing behind it: `a.river` was read once when the beast was set down, so **nothing on this earth ever went TO water**; the herds go down at dawn and dusk now, in four lands of four. **Everything else was ✅ because Round 54 READ `js/behavior.js`** — an entry whose own last words are "Measure first". Round 78 measured it (test 54) and the instrument convicted itself twice before it convicted the world: its "seven acts never seen" counted `graze`, which is a trade and not an act, and species that never stood in those lands; and its "49 never arrived home" was its own eighteen-frame clock against a walk wanting two hundred — held at one hour, 22 of 24 are abed by 400 frames. **The hours, the bed and the acts are all sound**, the only acts never seen being `drink` and `wallow` behind Round 69's `a.river` gate. What survived: **60 kinds declare a real home and 22 keep hours that never turn**, so `asleep` was hard-false for them and they were abed **0%** of a swept day while `js/nest.js` went on raising wolf earths and bear caves nothing slept in. A beast that keeps no hours now lies up once a day at an hour of its own — A/B/A in one boot, dusk **0 → 18 → 0%**, `all` 0 → 3 → 0%, day and night untouched, frame delta inside the off-to-off drift. Guards re-read: 35, 49 and 53 all clean. ~~**Still NOT built: the lion does not sleep**~~ — **STRUCK in Round 79, which measured it.** The claim was true of the FLAG and false of the WORLD: a stalking hunter with no quarry lies up in the deep grass, and the lion measures **truly still 100%** of 399 frames over two lands. He lacks `job==='bed'`, not a rest, and the behaviour he has is better because it leaves him in the cover the hunt needs. The elephant reads truly still **0%** — and that is what the data asks for, the file's own gloss on 'all' being "the elephant, who cannot afford to stop eating". **The world agrees with the file on both counts and nothing shipped.** Test 54's "0 kills" was its own false zero too — the sweep's 18-frame windows never let a chase finish; held in one land the hunt kills 4 times in 500 frames |
...
**PAID — ✅ Round 89, and by measurement, not by fiat.** `tools/farfalls.js` gave the
question its number and its picture: not five but 27 of 34 falls are basin-bound, EVERY
basin holds (0 cells outside any claim, farthest 6–73 blocks against claims of 46–376),
and the nearest true water lies 69–312 blocks off for fourteen of them and nowhere within
400 for eleven — so the channel would be the county-crossing cut this file's own law
